...
def write_to_file(**context):
    ...
    # Create a file with the name "Job_Search_<current day>"
    file_name = f'Job_Search_{today}.csv'
    df.to_csv(file_name, index=False)
...
dag = DAG('job_data_pipeline_plus_csv', default_args=default_args, description='A simple job data pipeline', schedule_interval=timedelta(days=1))
